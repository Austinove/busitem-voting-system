...
if (isset($_POST['name'])&&isset($_POST['pass'])) {
	...
	$pass = md5 ($_POST['pass']);
      ...
      // if (mysqli_num_rows($stopper)==0) {
        ...
        $result1 = ("SELECT * FROM after_vote_n WHERE reg_number = '$user'");
        $query1 = mysqli_query($connect, $result1);
        if (mysqli_num_rows($query1)==0) {
        $result = ("SELECT * FROM students_n WHERE reg_number = '$user' AND password = '$pass'");
        ...
        if (mysqli_num_rows($query)==1) {
          ...
          if ($user==$username&&$pass==$realpass) {
            $_SESSION['username_n']=$first .' '. $second;
            $_SESSION['reg']=$user;
            echo "Loading";
          }else {
         echo $_SESSION['error'] = "INCORRECT PASSWORD!!!";
          ...
          echo $_SESSION['error'] = "YOUR NOT REGISTERED!!!";
        }
      } else {
          echo $_SESSION['error'] = "YOU FINISHED VOTING!!!";
        }
    // } else {
...
}elseif (isset($_POST['name_a'])&&isset($_POST['pass_a'])) {
  $user = strtoupper($_POST['name_a']);
  $pass = md5 ($_POST['pass_a']);
      ...
      // if (mysqli_num_rows($stopper)==0) {
        ...
        if (mysqli_num_rows($query)==1) {
          while ($row = mysqli_fetch_assoc($query)) {
            $username_a = $row['reg_number'];
            $realpass_a = $row['password'];
            $first_a  = $row['first_name'];
            $second_a = $row['second_name'];
          }
          if ($user==$username_a&&$pass==$realpass_a) {
            $_SESSION['username_an']=$first_a .' '. $second_a;
            $_SESSION['reg_a']=$user;
            echo "Loading";
          }else {
         echo $_SESSION['error'] = "INCORRECT PASSWORD!!!";
  ...
  if (!empty($_POST['usernamesignup'])) {
    if (!empty($_POST['usernamesignup2'])) {
       if (!empty($_POST['regsignup'])) {
         if (!empty($_POST['emailsignup'])) {
            if (!empty($_POST['passwordsignup'])) {
              if (!empty($_POST['passwordsignup_confirm'])) {
                ...
                $reg = strtoupper($_POST['regsignup']);
              ...
              //           if ($email=='') {
                            ...
                            if ((strpos($first, $find)||strpos($first, $f)||strpos($first, $r)||strpos($first, $d)||strpos($first, $g))&&(strpos($second, $find)||strpos($second, $f)||strpos($second, $r)||strpos($second, $d)||strpos($second, $g))) {
                              echo $_SESSION['loaded']="Only Words Are Required";
                            } elseif (strpos($reg, $g)||strpos($reg, $f)||strpos($reg, $r)||strpos($reg, $d)) {

                                echo $_SESSION['loaded']="Registration Number Enter 'BU/UP/20__/0000' Or 'BU/UG/20__/0000'";

                              } elseif ((strpos($reg, $find)==2)&&(strpos($reg, $find, 3)==5)&&(strpos($reg, $find, 6)==10)) {
                                  if($pass === $passc) {
                                    $query = "SELECT * FROM all_students_n WHERE reg_number = '$reg'";
                                    $check=mysqli_query($connect, $query);
                                    if (mysqli_num_rows($check)==1) {

                                      $query = "SELECT * FROM students_n WHERE reg_number = '$reg'";
                                      $check=mysqli_query($connect, $query);
                                      if (mysqli_num_rows($check)<1) {
                                        ...
                                        if (strlen($pass1)>=8) {
                                            ...
                                            $_SESSION['reg'] = strtoupper($_POST['regsignup']);
...
class NexmoMessage {
  ...
  // Done!
                                ...
                                echo "Password Atleast 8 characters required";
                              }
                                } else {
                                echo "Already registered";
                              }
                              } else {
                                echo "Your not a Busitema student plz";
                              }
                            } else {
                              echo "Password don't match";
                            }
                          } else {
                            echo "Registration Number Enter 'BU/UP/20__/0000' Or 'BU/UG/20__/0000'";
                          }
  ...
  if (isset($_POST['token'])) {
    $realtoken = $_POST['token'];
    $snttoken = $_SESSION['token'];
    // echo $snttoken;
    if ($realtoken == '') {
      echo "Enter the token";
    } else {
      if ($realtoken == $snttoken) {
      
        $first = $_SESSION['first'];
        $second = $_SESSION['second'];
        $reg = $_SESSION['reg'];
        $pass = $_SESSION['pass'];
        $quer = "SELECT * FROM students_n WHERE reg_number = '$reg'";
        $chec=mysqli_query($connect, $quer);
        if (mysqli_num_rows($chec)==0) {
        
        $result = "INSERT INTO students_n (reg_number, first_name, second_name, password) VALUES ('$reg', '$first', '$second', '$pass')";
        if (mysqli_query($connect, $result)) {
          echo "Your registered login now";
          } else {
            echo "A problem occuired try again";
          }
 ...
 //                  $d = '"';
 //                  $g = "@";
 //                  if ((strpos($first, $find)||strpos($first, $f)||strpos($first, $r)||strpos($first, $d)||strpos($first, $g))){
 //                    echo "Only Words Are Required";
 //                  } else if((strpos($second, $find)||strpos($second, $f)||strpos($second, $r)||strpos($second, $d)||strpos($second, $g))) {
 //                    echo "Only Words Are Required";
 //                  } else if ((strpos($reg, $find)==2)&&(strpos($reg, $find, 3)==5)&&(strpos($reg, $find, 6)==10)) {
 //                    if($pass === $passc) {
 ...
 //                              }
 //                               else {
 //                                echo $_SESSION['loaded']="Pliz Try Again";
 //                              }
 //                  //           } else {
 //                  //             echo $_SESSION['error']='This Registration Number- '.$reg.' -Already Exists';
 //                  //           }
 //                  //       } else {
 //                  //         echo $_SESSION['error']="password don't match";
 //                  //       }
 //                  //     } else {
 //                  //       echo $_SESSION['error']="Registration Number Enter 'BU/UP/20__/0000' Or 'BU/UG/20__/0000'";
 //                        } else{
 //                          echo "Atleast 8 characters required";
 //                        }
 //                      } else {
 ...
 //      $sql = ("DELETE FROM after_vote");
 //      $query = mysqli_query($connect, $sql);

 //      $sql = ("DELETE FROM chairr");
 //      $query = mysqli_query($connect, $sql);

 //      $sql = ("DELETE FROM femalee");
 //      $query = mysqli_query($connect, $sql);

 //      $sql = ("DELETE FROM all_guild");
 //      $query = mysqli_query($connect, $sql);

 //      $sql = ("DELETE FROM guildd");
 //      $query = mysqli_query($connect, $sql);

 //      $sql = ("DELETE FROM malee");
 //      $query = mysqli_query($connect, $sql);

 //      $sql = ("DELETE FROM rccc");
 //      $query = mysqli_query($connect, $sql);

 //      $sql = ("DELETE FROM secc");
 //      $query = mysqli_query($connect, $sql);

 //      $sql = ("DELETE FROM spee");
 //      $query = mysqli_query($connect, $sql);
 ...
 //      $sql = ("DELETE FROM after_vote_n");
 //      $query = mysqli_query($connect, $sql);

 //      $sql = ("DELETE FROM chair_n");
 //      $query = mysqli_query($connect, $sql);

 //      $sql = ("DELETE FROM female_n");
 //      $query = mysqli_query($connect, $sql);

 //      $sql = ("DELETE FROM all_guild");
 //      $query = mysqli_query($connect, $sql);

 //      $sql = ("DELETE FROM guild_n");
 //      $query = mysqli_query($connect, $sql);

 //      $sql = ("DELETE FROM male_n");
 //      $query = mysqli_query($connect, $sql);

 //      $sql = ("DELETE FROM rcc_n");
 //      $query = mysqli_query($connect, $sql);

 //      $sql = ("DELETE FROM sec_n");
 //      $query = mysqli_query($connect, $sql);

 //      $sql = ("DELETE FROM spe_n");
 //      $query = mysqli_query($connect, $sql);

 //        echo "Deleting Complete";
      
 //    } elseif (isset($_POST['a_name'])) {
 //      $delete = strtoupper($_POST['a_name']);
 //      $result = ("SELECT * FROM admin WHERE reg_number = '$delete'");
 //      $query = mysqli_query($connect, $result);
 //      if (mysqli_num_rows($query)==1) {
 //       $sql = ("DELETE FROM admin WHERE reg_number = '$delete'");
 //       if ($query = mysqli_query($connect, $sql)) {
 //        echo '"'.$delete.'" Was Deleted';
 //       }
 //      } else {
 //        echo '"'.$delete.'" Is Not Registered';
 //      }
 //    }

// if (isset($_POST['vote'])) {
//    $_SESSION['continue'] = 'yes';
// }
?>